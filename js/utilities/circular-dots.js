class CircularDots {
    constructor(container, options = {}) {
        ...
        this.state = { activeIndex: this.options.activeIndex, animationFrame: null, startTime: null, isAnimating: false };
    ...
    setupEventListeners() {
        this.container.addEventListener('click', e => {
            ...
            if (this.options.onDotClick) this.options.onDotClick(index);
        });
    }
    ...
    setActiveDot(index, immediate = false) {
        ...
        dots.forEach(dot => {
            ...
            if (progress) { progress.style.strokeDashoffset = `${this.CIRCUMFERENCE}`; progress.style.transition = 'none'; }
        });
        ...
        if (!immediate) this.startAnimation(dots[index]);
        else {
            const progress = dots[index].querySelector('.circular-dot-progress-fill');
            if (progress) { progress.style.strokeDashoffset = '0'; progress.style.transition = 'none'; }
        }
    }
    ...
    startAnimation(dot) {
        ...
        const animate = () => {
            ...
            progress.style.strokeDashoffset = this.CIRCUMFERENCE - (this.CIRCUMFERENCE * progressAmount);

            if (progressAmount < 1) this.state.animationFrame = requestAnimationFrame(animate);
            else { this.state.isAnimating = false; this.state.animationFrame = null; if (this.options.onProgressComplete) this.options.onProgressComplete(this.state.activeIndex); }
        };
    ...
    restartAnimation() {
        ...
        if (activeDot && activeDot.classList.contains('active')) this.startAnimation(activeDot);
    }
}
